...
from combust.models import VoxelPolar1

from build import build_loader

# settings
settings_path = 'voxel_polar/voxel_polar.yml'
...
# data
train_gen, val_gen, irc_gen, test_gen, tr_steps, val_steps, irc_steps, test_steps, normalizer= build_loader(settings,device)
...
model = VoxelPolar1(model_type=settings['model']['mode'],
                   voxel_valtype='norm',
                   smearing_type='polar',
                   ...
                   smoothing_sigma=settings['model']['smoothing_sigma'],
                   smoothing_kernel=settings['model']['smoothing_kernel'],
                   # kernel_sizes=[(1, 1, 1)] + [(3, 3, 5)] * 3,  # ((3,3,5),(3,3,5),(3,3,5)),
                   # padding_sizes=[(0, 0, 0)] + [(1, 1, 2)] * 3,  # ((1,1,2),(1,1,2),(1,1,2)),
                   # out_channels=[2] * 3 + [1],  # (2,1,1),
                   pooling_type=settings['model']['pooling_type'],
                   max_z=10,
                   # n_atom_basis=64,
